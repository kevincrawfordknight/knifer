...
def beam_decode(model: AWDCharLSTM,
    ...
    # For homophonic mode, we'll need the raw cipher text
    # For now, use cleaned text but will fix after homophonic detection
    ...
    if homophonic:
        # For homophonic mode, use raw cipher text to determine cipher vocabulary
        ct_raw_clean = ct_raw.lower().strip()
    ...
    if force_first:
        ...
        if (homophonic and HASH_CIPHER >= 0 and c0i == HASH_CIPHER) or ((not homophonic) and HASH is not None and c0i == HASH):
            p0i = HASH
            if logp_after_prompt is not None:
...
def main():
    ...
    if scored:
        ...
        if args.homophonic > 0:
            ...
            cipher_text = read_all(args.cipherfile).lower().strip()
