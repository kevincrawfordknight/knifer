...
def main():
    ...
    ap.add_argument('--alphabet', type=str, default="#abcdefghijklmnopqrstuvwxyz",
                    help="Character set to keep; default 27 with '#'. Use 'abcdefghijklmnopqrstuvwxyz' for 26.")
    ...
    # Build vocab from requested alphabet (overridden later if resuming with a checkpoint-specified vocab)
    alphabet = args.alphabet
    stoi, itos = build_vocab(alphabet)

    # If resuming, inspect checkpoint to decide model shapes & possibly override alphabet
    resume_sd = None
    start_epoch = 1
    ...
    if args.resume:
        ...
        # infer shapes and (maybe) vocab
        vocab_size, emb_r, hidden_r, layers_r = _infer_shapes_from_state(sd)
        if 'vocab' in ck and isinstance(ck['vocab'], str):
            alphabet = ck['vocab']
            stoi, itos = build_vocab(alphabet)
        else:
