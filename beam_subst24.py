#!/usr/bin/env python3
"""
beam_subst24.py — Language-agnostic beam search for monoalphabetic and homophonic substitution.
Based on beam_subst23.py.
"""
...
def compute_rest_cost_single(model, alphabet: str, ct: str, current_pos: int,
...
def compute_rest_cost_batched(model, alphabet: str, ct: str, current_pos: int,
                             states_data: list, device, M: int = 0, micro: int = 8192, cipher_A2I: dict = None) -> list:
    """
    Batched version of compute_rest_cost for multiple beam states.
    states_data: list of (h_state, c2p_arr, p2c, last_char_idx, homophonic_limit) tuples
    Returns: list of (cost, sampled_text) tuples
    """
    if M <= 0:
        return [(0.0, "") for _ in states_data]

    # Process in micro-batches
    all_results = []

    for start in range(0, len(states_data), micro):
        end = min(len(states_data), start + micro)
        batch = states_data[start:end]
        ...
        remaining_length = len(ct) - current_pos
        if remaining_length <= 0:
            all_results.extend([(0.0, "") for _ in batch])
            continue

        lookahead_len = min(M, remaining_length)

        # Stack hidden states for batching
        h_states = []
        c_states = []
        ...
        for h_state, _, _, _, _ in batch:
            if h_state is None:
                # Handle None state - use zeros with correct dtype
                layers = 3  # Default fallback
                hidden_size = 512  # Default fallback
                h_states.append(torch.zeros(layers, hidden_size, device=device, dtype=model_dtype))
                c_states.append(torch.zeros(layers, hidden_size, device=device, dtype=model_dtype))
            else:
                # Ensure hidden states match model dtype
                h_states.append(h_state[0].to(dtype=model_dtype))
                c_states.append(h_state[1].to(dtype=model_dtype))

        if h_states:
            h_batch = torch.stack(h_states, dim=1)  # [layers, batch, hidden]
            c_batch = torch.stack(c_states, dim=1)
            h_current = (h_batch, c_batch)
        else:
            h_current = None

        # Get last characters for each state
        last_chars = [data[3] for data in batch]  # last_char_idx

        # Prepare available characters for each state
        A2I = {c: i for i, c in enumerate(alphabet)}
        candidate_indices_batch = []

        for _, c2p_arr, p2c, _, homophonic_limit in batch:
            if p2c is not None:
                candidate_indices = []
                for c in alphabet:
                    if c != '#':
                ...
                candidate_indices = [A2I[c] for c in alphabet if c != '#']
            candidate_indices_batch.append(candidate_indices)

        # Initialize batch results
        total_costs = [0.0] * batch_size
        sampled_texts = [""] * batch_size

        with torch.no_grad():
            for i in range(lookahead_len):
                # Feed last characters to get next-character probabilities
                last_input = torch.tensor(last_chars, dtype=torch.long, device=device)
                logits, h_current = model(last_input.view(batch_size, 1), h_current)
                logp_batch = F.log_softmax(logits[:, -1, :], dim=-1)  # [batch, vocab]

                # Process each state in the batch
                new_last_chars = []
                for j in range(batch_size):
                    _, c2p_arr, _, _, _ = batch[j]
                    logp = logp_batch[j]
                    candidate_indices = candidate_indices_batch[j]
                    ...
                    if next_cipher_pos < len(ct):
                        ...
                        if next_cipher_char in cipher_A2I:
                            ...
                            if cipher_idx < len(c2p_arr) and c2p_arr[cipher_idx] != -1:
                                ...
                                # Not committed - find best among candidates
                                best_logp = float('-inf')
                                best_idx = candidate_indices[0] if candidate_indices else 0
                                for idx in candidate_indices:
                                    if logp[idx].item() > best_logp:
                                        best_logp = logp[idx].item()
                                        best_idx = idx
                        else:
                            # Cipher char not in vocab - find best among candidates
                            best_logp = float('-inf')
                            best_idx = candidate_indices[0] if candidate_indices else 0
                            for idx in candidate_indices:
                                if logp[idx].item() > best_logp:
                        ...
                        # Past end of cipher text - find best among candidates
                        best_logp = float('-inf')
                        best_idx = candidate_indices[0] if candidate_indices else 0
                        for idx in candidate_indices:
                            if logp[idx].item() > best_logp:
                    ...
                    total_costs[j] += best_logp
                    sampled_texts[j] += alphabet[best_idx]
                    new_last_chars.append(best_idx)

                # Update last_chars for next iteration
                last_chars = new_last_chars

        # Add batch results to overall results
        for j in range(batch_size):
            all_results.append((total_costs[j], sampled_texts[j]))

    return all_results

def compute_rest_cost_multiple(requests, model, alphabet: str, ct: str, current_pos: int, cipher_A2I: dict, device, M: int = 0, micro: int = 8192):
    """
    Compute restcost for multiple requests in batches.
    requests: list of (h_state, c2p_arr, p2c, last_char_idx, homophonic_limit) tuples
    Returns: list of (cost, sampled_text) tuples
    """
    if M <= 0:
    ...
    return compute_rest_cost_batched(model, alphabet, ct, current_pos, states_data, device, M, micro, cipher_A2I)

def compute_rest_cost(model, alphabet: str, ct: str, current_pos: int,
                     h_state, c2p_arr: array, cipher_A2I: dict, device, M: int = 0, last_char_idx: int = 0, p2c=None, homophonic_limit: int = 1) -> tuple:
    """
    Wrapper that calls the single version for backward compatibility.
    """
    return compute_rest_cost_single(model, alphabet, ct, current_pos, h_state, c2p_arr, cipher_A2I, device, M, last_char_idx, p2c, homophonic_limit)
...
def beam_decode(model: AWDCharLSTM,
    ...
    c0 = ct[0]
    c0i = cipher_A2I[c0]
    seed_states = []  # List of (BeamState, restcost_request) tuples
    ...
    def add_seed(p0i: int, g0: float, h0):
        ...
        if hash_cipher_idx == -1 or c0i != hash_cipher_idx:
            c2p[c0i] = p0i
            p2c[p0i].add(c0i)
        # Create seed state without restcost initially
        st = BeamState(g=g0, h=0.0, node=node0, c2p=c2p, p2c=p2c, h_prev=h0, last_idx=p0i, length=1, rest="")
        # Store restcost request for batch processing
        restcost_request = (h0, c2p, p2c, p0i, homophonic_limit)
        seed_states.append((st, restcost_request))
    ...
    # Batch compute restcosts for seed states
    seed_requests = [req for _, req in seed_states]
    seed_restcost_results = compute_rest_cost_multiple(seed_requests, model, alphabet, ct, 1, cipher_A2I, device, restcost, micro)

    # Update seed states with computed restcosts
    final_seed_states = []
    for i, (seed_state, _) in enumerate(seed_states):
        h_new, rest_new = seed_restcost_results[i]
        seed_state.h = h_new
        seed_state.rest = rest_new
        final_seed_states.append(seed_state)

    final_seed_states.sort(key=lambda s: rank_score_astar(s.g, s.h, s.length, alpha), reverse=True)
    states = final_seed_states[:beam_size]
        ...
        if pt_debug_clean:
            ...
            if target_len <= len(pt_debug_clean):
                ...
                pt_bpc = conditional_bpc(model, alphabet, prompt, pt_prefix,
                                       p0_log if prompt == "" else None, str(device))
                if best_st.rest:
                    print(f"bpc={best_bpc:.3f} (pt bpc={pt_bpc:.3f}) {best_pt_so_far}[{best_st.rest}] s.g={best_st.g:.3f} s.h={best_st.h:.3f}")
                else:
                    print(f"bpc={best_bpc:.3f} (pt bpc={pt_bpc:.3f}) {best_pt_so_far} s.g={best_st.g:.3f} s.h={best_st.h:.3f}")
            else:
                if best_st.rest:
                    print(f"bpc={best_bpc:.3f} {best_pt_so_far}[{best_st.rest}] s.g={best_st.g:.3f} s.h={best_st.h:.3f}")
                else:
                    print(f"bpc={best_bpc:.3f} {best_pt_so_far} s.g={best_st.g:.3f} s.h={best_st.h:.3f}")
        else:
            if best_st.rest:
                print(f"bpc={best_bpc:.3f} {best_pt_so_far}[{best_st.rest}] s.g={best_st.g:.3f} s.h={best_st.h:.3f}")
            else:
    ...
    for pos in range(1, len(ct)):
        ...
        # Collect all candidate states and their restcost requests
        ...
        for i, st in enumerate(states):
            ...
            if hash_cipher_idx >= 0 and ci == hash_cipher_idx:
                ...
                # Create candidate without restcost initially
                st2 = BeamState(g=g_new, h=0.0, node=node2, c2p=st.c2p, p2c=st.p2c,
                                h_prev=hrow, last_idx=pi, length=st.length+1, rest="")
                # Store restcost request
                restcost_request = (hrow, st.c2p, st.p2c, pi, homophonic_limit)
                candidates_with_requests.append((st2, restcost_request))
                continue
            ...
            if inside_prefix:
                ...
                if st.c2p[ci] == -1:
                    ...
                    g_new = st.g + float(row[pi].item())
                    # Create candidate without restcost initially
                    st2 = BeamState(g=g_new, h=0.0, node=node2, c2p=c2p_new, p2c=p2c_new,
                                    h_prev=hrow, last_idx=pi, length=st.length+1, rest="")
                    # Store restcost request
                    restcost_request = (hrow, c2p_new, p2c_new, pi, homophonic_limit)
                    candidates_with_requests.append((st2, restcost_request))
                else:
                    # Already mapped
                    node2 = nodes.add(st.node, pi)
        ...
        # Phase 2: Batch compute all restcosts and update candidates
        if candidates_with_requests:
            # Extract restcost requests
            requests = [req for _, req in candidates_with_requests]

            # Batch compute restcosts
            restcost_results = compute_rest_cost_multiple(requests, model, alphabet, ct, pos+1, cipher_A2I, device, restcost, micro)

            # Update candidates with computed restcosts
            for i, (candidate_state, _) in enumerate(candidates_with_requests):
                h_new, rest_new = restcost_results[i]
                candidate_state.h = h_new
                candidate_state.rest = rest_new

                # Add to final candidates list with A* score
                candidates.append((rank_score_astar(candidate_state.g, candidate_state.h, candidate_state.length, alpha), candidate_state))
...
def main():
    ...
    ap.add_argument("--incremental", action="store_true", help="show incremental triangle output during beam search (uses s.g+s.h)")
