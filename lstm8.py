#!/usr/bin/env python3
# lstm8.py — AWD char-LSTM with memory-efficient streaming data loading
...
class StreamingCharDataset(torch.utils.data.IterableDataset):
    """
    Memory-efficient streaming dataset that reads file in chunks.
    Only keeps one chunk in memory at a time.
    """
    def __init__(self, path: str, alphabet: str, stoi: dict, block: int, chunk_size: int = 10_000_000):
        self.path = path
    ...
    def __iter__(self):
        with io.open(self.path, "r", encoding="utf8", errors="ignore") as f:
            buffer = ""
            while True:
                ...
                # Process this chunk
                buffer += chunk.lower()
                buffer = clean_to_alphabet(buffer, self.alphabet)

                # Convert to IDs and yield blocks
                while len(buffer) >= self.block + 1:
                    # Extract one block + target
                    block_text = buffer[:self.block]
                    target_text = buffer[1:self.block+1]

                    x = torch.tensor([self.stoi[c] for c in block_text], dtype=torch.long)
                    y = torch.tensor([self.stoi[c] for c in target_text], dtype=torch.long)
    ...
    def __len__(self):
        # Approximate length - exact calculation would require reading entire file
        estimated_chars = self.file_size // 2  # Rough estimate after cleaning
        return max(0, (estimated_chars - 1) // self.block)

def load_text_ids(path: str, alphabet: str, stoi: dict) -> torch.Tensor:
    """Legacy function - now only used for small test sets"""
    with io.open(path, "r", encoding="utf8", errors="ignore") as f:
        s = f.read().lower()
...
def evaluate_streaming(model: AWDCharLSTM, loader, device: str) -> Tuple[float,float]:
    """Evaluate using a pre-built dataloader (for streaming datasets)"""
    ce = nn.CrossEntropyLoss(reduction="sum")
    model.train(False)
    total_loss = 0.0
    total_tok = 0
...
def main():
    ...
    # Determine if we should use streaming (for large files) or legacy loading
    train_file_size = os.path.getsize(args.train)
    test_file_size = os.path.getsize(args.test)

    # Use streaming for files larger than 1GB
    use_streaming_train = train_file_size > 1_000_000_000
    use_streaming_test = test_file_size > 1_000_000_000

    if use_streaming_train:
        print(f"Using streaming dataset for large training file ({train_file_size/1e9:.1f}GB)")
        train_ds = StreamingCharDataset(args.train, alphabet, stoi, args.block)
        train_ids = None  # Not loaded into memory
    else:
        print(f"Using legacy loading for training file ({train_file_size/1e6:.1f}MB)")
        train_ids = load_text_ids(args.train, alphabet, stoi)
        train_ds = CharStream(train_ids, args.block)

    if use_streaming_test:
        print(f"Using streaming dataset for large test file ({test_file_size/1e9:.1f}GB)")
        test_ds = StreamingCharDataset(args.test, alphabet, stoi, args.block)
        test_ids = None  # Not loaded into memory
    else:
        print(f"Using legacy loading for test file ({test_file_size/1e6:.1f}MB)")
    ...
    # Ensure parent directory exists for final save path
    ...
    for e in range(start_epoch, args.epochs + 1):
        ...
        if use_streaming_test:
            # For streaming datasets, use the dataloader directly
            ebpc, eppl = evaluate_streaming(model, test_loader, device=device)
        else:
            # For legacy datasets, use the tensor directly
