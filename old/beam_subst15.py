...
# ---- from score4 loader (lightweight copy) ----
...
def run_prompt(model: AWDCharLSTM, prompt: str, alphabet: str):
    ...
    for t in range(1, len(s)):
        ...
        _, h = model(x, h)
    # logits for next char after last prompt char
    ...
    # squeeze batch dim from h_after (L,1,H) -> (L,H)
...
def beam_decode(model: AWDCharLSTM,
    ...
    HASH = AI["#"] if alphabet[0] == "#" else None

    ct = clean_text(ct, alphabet)
    if len(ct) == 0:
    ...
    def add_seed(p0i: int, g0: float, h0):
        ...
        if c2p[p0i] == -1:  # allow mapping unless fixed hash
            c2p[c0i] = p0i
            p2c[p0i] = c0i
    ...
    if HASH is not None and c0i == HASH:
        ...
        if logp_after_prompt is not None:
            g0 = float(logp_after_prompt[p0i].item())
            h0 = h_after_prompt
        else:
            if p0_log is None:
                raise ValueError("Need P0 when no prompt is provided")
            g0 = float(p0_log[p0i].item())
            h0 = None
        add_seed(p0i, g0, h0)
    else:
        if logp_after_prompt is not None:
            ...
            # Do not allow '#'(HASH) unless ciphertext is '#'
    ...
    for pos in range(1, len(ct)):
        ...
        for i, st in enumerate(states):
            ...
            if mapped != -1:
                ...
                # new assignment: choose topk_expand unused plaintext symbols (excluding '#')
                scores = row.clone()
                # optionally add heuristic letter prior for exploration (NOT at t=0; this is t>0)
                # We could mix in an ETAOIN log prior here via prior_w if desired; for simplicity keep scores as-is,
                # and optionally add small prior_w*etaoin later. We'll keep small prior use:
                if prior_w > 0.0:
                    # make a tiny ETAOIN log prior over the alphabet
...
def main():
    ...
    ap.add_argument("--prior_w", type=float, default=0.10, help="small heuristic letter prior for NEW assignments at t>0")
    ...
    # Prior P0 controls (used only if prompt=="")
    ap.add_argument("--prior_text", type=str, default="", help="estimate P0 from text file")
    ...
    # Build P0 (only used when no prompt)
    if args.prior_text:
    ...
    best_pt, c2p_full, g_raw, pack = beam_decode(
        ...
        prefix=args.prefix, prompt=args.prompt, p0_log=p0_log if args.prompt == "" else None
    )
    states, nodes = pack

    # N-best (sorted by conditional bpc with the same boundary policy)
    ...
    # Simple refinement: try all pair swaps among seen ciphertext letters (1 pass)
    if args.refine > 0:
        # make plaintext from best key
        table = [alphabet[p] for p in c2p_full]
        pt0 = "".join(table[alphabet.index(ch)] for ch in clean_text(ct, alphabet))
        # very light local search: no swaps implemented here to keep code concise

        bpc_best = conditional_bpc(model, alphabet, args.prompt, pt0,
                                   p0_log if args.prompt == "" else None, device=str(device))
        print("PLAINTEXT:\n", pt0)
        print("BPC:", bpc_best)
        print("KEY c->p:\n", "".join(alphabet[p] for p in c2p_full))
    else:
        bpc_best = conditional_bpc(model, alphabet, args.prompt, best_pt,
                                   p0_log if args.prompt == "" else None, device=str(device))
        print("PLAINTEXT:\n", best_pt)
        print("BPC:", bpc_best)
        print("KEY c->p:\n", "".join(alphabet[p] for p in c2p_full))
