#!/usr/bin/env python3
"""
beam_subst14.py — Memory-tight beam search for monoalphabetic substitution (# + a..z),
with GPU micro-batching and mixed precision.

Adds:
  • --micro  : process beam in chunks to cap peak GPU memory
  • --device : 'cuda' or 'cpu'
  • --dtype  : 'fp32' (default), 'fp16', 'bf16' (GPU only)

Keeps v13 improvements:
  • backpointer plaintext (no per-beam strings)
  • int8 c2p/p2c
  • prompt-aware scoring
  • optional refinement
"""
...
from score2_lstm import AWDCharLSTM, load_model  # auto-detect loader keeps '#'
...
def bpc_for_text_local(model: AWDCharLSTM, text: str, device: str = "cuda") -> float:
    ...
    total_nll = 0.0; total_tok = 0; h = None
    for t in range(1, ids.size(1)):
        x = ids[:, t-1:t]
        logits, h = model(x, h)
        logp = F.log_softmax(logits[0, -1, :], dim=-1)
        total_nll += float(-logp[ids[0, t]].item()); total_tok += 1
    return (total_nll / total_tok) / math.log(2)

# --------------- Micro-batched one step ---------------

@torch.no_grad()
def one_step_micro(model: AWDCharLSTM,
                   states: List[BeamState],
                   micro: int,
                   prior_vec_dev: torch.Tensor,
                   gumbel: float,
                   ):
    """
    Returns per-beam row_logp list and h_after views, computed in chunks of size 'micro'.
    """
    device = next(model.parameters()).device
    B = len(states)
    row_logps = [None]*B
    ...
    # Decide dtype context
    use_cuda = (device.type == "cuda")
    # We don't force autocast here; model dtype controls precision.
    ...
    for start in range(0, B, micro):
        ...
        logits_next, h_after = model(last_idx.view(-1,1), h_prev_batch)  # logits [M,1,V]
        logp_next = F.log_softmax(logits_next[:, -1, :], dim=-1)        # [M,V]

        # Split into per-row tensors (views)
        for j in range(end-start):
            row_logps[start+j] = logp_next[j]
            h_rows[start+j] = (h_after[0][:, j, :], h_after[1][:, j, :])

    return row_logps, h_rows

# --------------- Beam search (micro-batched) ---------------

@torch.no_grad()
def beam_decode_v14(model: AWDCharLSTM,
                    ct: str,
                    beam_size: int = 800,
                    topk_expand: int = 12,
                    prior_w: float = 0.10,
    ...
    for pos in range(1, T):
        ...
        row_logps, h_rows = one_step_micro(model, states, micro, prior_vec, gumbel)

        cand_rank: List[Tuple[float, BeamState]] = []

        for i, st in enumerate(states):
            row_logp = row_logps[i]
            h_row = h_rows[i]
            ...
            if mapped != -1:
                ...
                if forced_pi is not None:
                    ...
                    scores = row_logp + prior_w * LOGP_PRI.to(row_logp.device)
        ...
        # optional: free cached blocks to fight fragmentation
    ...
    for _ in range(max(0, rounds)):
        ...
        for ii, i_c in enumerate(present_idx):
            for j_c in present_idx[ii+1:]:
                if i_c == j_c: continue
                k2 = key[:]; k2[i_c], k2[j_c] = k2[j_c], k2[i_c]
...
def main():
    ...
    ap.add_argument("--micro", type=int, default=8192, help="max beams per GPU forward step")
    ...
    best_pt, c2p_full, g, pack, pt_falloff, prompt_clean = beam_decode_v14(
        model, ct,
        beam_size=args.beam,
        micro=args.micro,
    ...
    for st in topk_states:
        ...
        bpc = bpc_for_text(model, prompt_clean, pt, device=str(device))
    ...
    # refine
    if args.refine > 0:
        pt_final, c2p_final, bpc = refine_swaps(model, ct, c2p_full, prompt=prompt_clean, rounds=args.refine, device=str(device))
        print("PLAINTEXT:\n", pt_final)
        print("BPC:", bpc)
        print("KEY c->p:\n", "".join(ALPH[p] for p in c2p_final))
    else:
        bpc = bpc_for_text(model, prompt_clean, best_pt, device=str(device))
        print("PLAINTEXT:\n", best_pt)
        print("BPC:", bpc)
