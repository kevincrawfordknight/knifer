#!/usr/bin/env python3
"""
beam_subst8.py — Faster neural-LM beam search for monoalphabetic substitution.
...
New in this version:
  ...
  • --pt "<actual-plaintext>": NOT used for decoding; we report the 0-based
    index at which that plaintext prefix falls off the beam (or -1 if never)
...
Usage example:
  python beam_subst8.py char_lstm.pt ciphertext.txt \
    ...
    --nbest 10 --refine 2 --seed 0 --pt "iamthewalrus..."
"""
...
def beam_decode_v8(model: AWDCharLSTM,
    ...
    if pt_actual and len(pt_actual) != T:
        # clamp to min length for comparison; reporting is per-position anyway
...
def main():
    ...
    # Beam decode
    pt_beam, c2p_full, g, end_states, pt_falloff = beam_decode_v8(
        model, ct,
        beam_size=args.beam,
    ...
    # Evaluate BPC for top-N and print one line per entry: "<bpc:.3f> <plaintext>"
